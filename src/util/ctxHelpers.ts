...
export const getHeaderValue = (context: ParameterizedContext, headerKeyToGet: string): string | undefined => {
	...
	if (typeof headerValue === 'string') {
		...
		console.warn(`unexpected value for ctx.headers.${headerKeyToGet}: ${util.format(headerValue)}`);
		return undefined;
	}
};
...
export const getQueryValue = (context: ParameterizedContext, queryKeyToGet: string): string | undefined => {
	...
	if (typeof queryValue === 'string') {
		...
		console.warn(`unexpected value for ctx.query.${queryKeyToGet}: ${util.format(queryValue)}`);
		return undefined;
	}
};
...
export const getStateValue = (context: ParameterizedContext, stateKeyToGet: string): string | undefined => {
	...
	if (typeof stateValue === 'string') {
		...
		console.warn(`unexpected value for ctx.state.${stateKeyToGet}. The whole object: ${util.format(context)}`);
		return undefined;
	}
};
